...
def filter_foods(user_input):
    ...
    meal_type = user_input.get("meal_type", "").lower()
    ...
    for f in FOODS:
        ...
        food_meal_types = [
            mt.lower()
            for mt in f.get("meal_type", [])
        ]

        if meal_type and meal_type not in food_meal_types:
            continue
...
Here are the ONLY meals you may choose from:
{meal_details}
...
MEAL: <name>
INGREDIENTS: <comma separated full ingredient list from the selected meal>
MACROS: Calories=<cal>, Protein=<g>, Carbs=<g>, Fat=<g>
REASON: <one clear sentence explaining how the meal satisfies the user's dietary preferences and avoids their selected allergens; optionally mention meal type or ingredients>

STRICT RULES:
- Choose only from the meals listed above
- Do NOT include ingredients that are not listed in the selected meal
- Do NOT mention user ingredients unless they appear in the selected meal
- Do NOT mention excluded ingredients
- Do NOT hallucinate or add extra ingredients
- Do NOT include multiple meals
- Do NOT include extra explanation
- If the user did not provide ingredients, do not say "ingredients provided"
- You MUST prioritize explaining dietary preferences and allergy avoidance in the reason
- Do NOT base the reason primarily on macros unless the user explicitly requested it
"""
    ...
    if not foods:
        return {
            "output": (
                ...
                "or meal type. Please try different inputs."
            ),
            "meal": "None",
            "retrieved_docs": [],
            "similarities": []
